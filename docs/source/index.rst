...
.. versionchanged:: v0.2.0: Hotfix for new ESO ETC server.

    The new ESO ETC server uses different units, which are now used.

.. versionchanged:: v0.1.5: Hotfix to replace ESO ETC server URL.

    Skycalc now uses https://etimecalret-002.eso.org as server, #25.

.. versionchanged:: v0.1.4: Drop support for Python 3.7.

    Skycalc now uses `pyproject.toml`, and only supports Python 3.8+.

.. versionchanged:: v0.1.3: Updated default values for `wmax`, `wgrid_mode`, `wres`
...
.. versionchanged:: v0.1.3: No longer saves a temporary FITS file to disk.
...
.. versionchanged:: v0.1.2: Upgrade fixes PyYaml warning/error for PyYaml >=6.0
